...
float cube_transparency = 0.1f; //transparency of "off" LEDs
			...
			else //If  WaitCommEvent()==True Read the RXed data using ReadFile();
			{
				...
				while (1 == 1)
				{
					...
					for (j = 0; j < i - 1; j++)		// j < i-1 to remove the dupliated last character
						display_array[0][j] = SerialBuffer[j];
					...
					if (TempChar >= 0xf0)
					{
						...
						char slayer = TempChar - 0xf0;

						


						for (char j = 0; j < i - 1; j++)		// j < i-1 to remove the dupliated last character
						{
							display_array[slayer][j] = SerialBuffer[j];
						}
					}
